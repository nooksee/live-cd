...
echo "
 Main options:
     ...
     -b|--theme     Install Plymouth Theme
     -l|--plymouth  Preview Plymouth Theme
     -u|--ubiquity  Ubiquity Editor
     -w|--usb       Image File to USB
     -f|--burn      Image File to Disk
...
Links:
  ...
  Homepage: http://nooksee.com
  Documentation: http://nooksee.com
  Issues: http://nooksee.com
...
if [ "$#" != "0" ];then
	for arg in "$@"; do
		case $arg in
			...
			-b|--theme) Root_it /etc/live-cd/scripts/theme ;;
			-l|--plymouth) Root_it /etc/live-cd/scripts/plymouth ;;
			-u|--ubiquity) Root_it /etc/live-cd/scripts/ubiquity ;;
			-w|--usb) Root_it /etc/live-cd/scripts/usb ;;
			-f|--burn) Root_it /etc/live-cd/scripts/burn ;;
			-v|--version) Version ;;
			-h|--help) Usage ;;
